...
def get_random_data(annotation_line, input_shape, random=True, max_boxes=20,
    ...
    if not random:
        ...
        # 将边界框box等比例缩小，再加上填充的偏移量dx和dy，因为新的图片部分用灰色填充，影响box的坐标系，
        # box最多有max_boxes个，即20个
    ...
    # 最终，返回图像数据image_data和边框数据box_data。
    # box的4个值是(xmin, ymin, xmax, ymax)，第5位不变，是标注框的类别，如0~n
    return image_data, box_data
...
def letterbox_image(image, size):
    """将图片缩放到目标尺寸，不改变其原始比例，多出的部分用灰色代替，见my_learning。
