...
class Yolo(object):
    def __init__(self, weights_file):
        ...
        self.conf_threshold = 0.2  # confidence scores threshold
    ...
    def _build_net(self):
        ...
        net = self._fc_layer(net, 27, self.S*self.S*(self.C+5*self.B))  # 7*7*(20+5*2)
        self.predicts = net

    def _conv_layer(self, x, layer_id, num_filters, filter_size, stride):
        """Conv layer"""
        ...
        if self.verbose:
            print("Layer %d: type=Conv, num_filter=%d, filter_size=%d, stride=%d, output_shape=%s"
                  % (layer_id, num_filters, filter_size, stride, str(output.get_shape())))
        return output

    def _fc_layer(self, x, layer_id, num_out, activation=None):
        ...
        if self.verbose:
            print("Layer %d: type=Fc, num_out=%d, output_shape=%s"
                  % (layer_id, num_out, str(output.get_shape())))
        return output

    def _maxpool_layer(self, x, layer_id, pool_size, stride):
    ...
    def detect_from_file(self, image_file, imshow=True, deteted_boxes_file="result.txt", detected_image_file="result.jpg"):
        """Do detection given a image file"""
        # read image
        image = cv2.imread(image_file)
        img_h, img_w, _ = image.shape
        predicts = self._detect_from_image(image)
        predict_boxes = self._get_bounding_boxes(predicts, img_h, img_w)
    ...
    def _detect_from_image(self, image):
        ...
        predicts = self.sess.run(self.predicts, feed_dict={self.images: _images})  # (1, 1470)
        predicts = predicts[0]  # (1470, )
        return predicts

    def _get_bounding_boxes(self, predicts, img_h, img_w):
    ...
    def show_results(self, image, results, imshow=True, deteted_boxes_file=None, detected_image_file=None):
...
if __name__ == "__main__":
    ...
    yolo_net.detect_from_file("./test_images/car.jpg")
